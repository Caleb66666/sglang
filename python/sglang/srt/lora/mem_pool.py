...
class LoRAMemoryPool:
    ...
    def __init__(
        ...
        #   (stacked_num, max_loras_per_batch, output_dim, max_lora_rank)
    ...
    def init_buffers(
        ...
        if self.max_lora_rank is None:
            ...
            assert self.max_lora_rank == max_lora_rank, (
            f"max_lora_rank {max_lora_rank} is not expected to change after initialization. Please file an issue if you see this error."
            )
        ...
        def update_buffer(
            ...
            get_lora_shape_fn: Callable[[str, torch.nn.Module], Tuple[int]],
        ):
            new_weight_names = lora_weight_names - buffer.keys()
            for module_name in new_weight_names:
                lora_shape = get_lora_shape_fn(module_name, base_model)
                buffer[module_name] = [
                    torch.zeros(
                        lora_shape,
                        dtype=self.dtype,
